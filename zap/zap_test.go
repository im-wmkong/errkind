...
import (
	...
	zapext "github.com/im-wmkong/errkind/zap"
	"go.uber.org/zap"
...
func TestLogging(t *testing.T) {
	...
	for _, err := range []error{nil, errors.New("plain"), tree} {
		...
		logger := zap.New(zapcore.NewCore(zapcore.NewJSONEncoder(zap.NewProductionEncoderConfig()), zapcore.AddSync(&buf), zapcore.DebugLevel))
		logger.Error("failed", zapext.Err(err), zapext.Object("custom", err))
